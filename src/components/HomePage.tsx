...
interface HomePageProps {
  /**
   * Blogs préchargés à afficher (mode SSR)
   */
  initialBlogs?: Blog[];
  
  /**
   * Si true, le composant va charger les blogs côté client
   */
  clientSideFetching?: boolean;
}

export default function HomePage({ initialBlogs = [], clientSideFetching = false }: HomePageProps) {
  const [blogs, setBlogs] = useState<Blog[]>(initialBlogs);
  const [loading, setLoading] = useState(clientSideFetching);

  useEffect(() => {
    // Ne charge les blogs du côté client que si clientSideFetching est activé
    if (clientSideFetching) {
