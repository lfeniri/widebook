...
export const NavComponentConfigModal: React.FC<NavComponentConfigModalProps> = ({ config, onSave, onClose }) => {
  ...
  const [links, setLinks] = useState<{ label: string; href: string }[]>(safeConfig.links || []);
  const [backgroundColor, setBackgroundColor] = useState<string>(safeStyle.backgroundColor || "#fff");
  const [color, setColor] = useState<string>(safeStyle.color || "#222");
  const [ariaLabel, setAriaLabel] = useState<string>(safeConfig.ariaLabel || "");
  const [padding, setPadding] = useState<string>(safeStyle.padding || "1rem");
  const [margin, setMargin] = useState<string>(safeStyle.margin || "0px");
  const [borderRadius, setBorderRadius] = useState<string>(safeStyle.borderRadius || "0.5rem");
  const [boxShadow, setBoxShadow] = useState<string>(safeStyle.boxShadow || "none");
  const [border, setBorder] = useState<string>(safeStyle.border || "none");

  const handleLinkChange = (idx: number, key: 'label' | 'href', value: string) => {
    setLinks((prev: { label: string; href: string }[]) =>
      prev.map((link, i) => (i === idx ? { ...link, [key]: value } : link))
    );
  };
  ...
  const handleAddLink = () => {
    setLinks((prev: { label: string; href: string }[]) => [...prev, { label: '', href: '' }]);
  };
