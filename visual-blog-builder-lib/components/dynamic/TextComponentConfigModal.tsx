...
export const TextComponentConfigModal: React.FC<TextComponentConfigModalProps> = ({ config, onSave, onClose }) => {
  ...
  return (
    <div className="space-y-4 p-4">
      <label className="flex flex-col gap-1">
        Texte
        <textarea className="border rounded p-2" value={text} onChange={e => setText(e.target.value)} />
      </label>
      <label className="flex flex-col gap-1">
        Couleur
      ...
      <label className="flex flex-col gap-1">
        Taille (px)
        <input type="number" min="10" max="80" value={parseInt(fontSize)} onChange={e => setFontSize(e.target.value + "px")}/>
      </label>
      <label className="flex flex-col gap-1">
        Police
